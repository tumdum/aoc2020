...
fn matches(s: &str, all: &HashMap<usize, Rule>) -> bool {
    matching_tails(&all[&0], s, all).iter().any(|m| *m == "")
}
...
#[test]
fn tests() {
    assert_eq!((3, Rule::Lit("b".to_owned())), parse("3: \"b\""));
    ...
    assert_eq!(
        vec!["b"],
        matching_tails(&Rule::Lit("a".to_owned()), "ab", &HashMap::new())
    );
    assert!(matching_tails(&Rule::Lit("a".to_owned()), "bab", &HashMap::new()).is_empty());
}
