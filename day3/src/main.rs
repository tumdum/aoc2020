...
fn get(input: &[Vec<char>], pos: &Pos) -> Option<char> {
    input.get(pos.y).and_then(|l| l.get(pos.x % l.len()).cloned())
}
...
fn walk(input: &[Vec<char>], start: &Pos, slope: &Pos) -> Vec<char> {
    ...
    loop {
        match get(&input, &start) {
            Some(c) => ret.push(c),
            None => break,
        }
        start = Pos{x: start.x+slope.x, y: start.y+slope.y};
    }
    ret
}

fn solve_a(input: &[Vec<char>], start: &Pos, slope: &Pos) -> usize {
    walk(&input, start, slope).into_iter().filter(|c| *c == '#').count()
}
